...
mod accepted_view_tests {
    ...
    async fn read_returns_accepted_view_with_footer() {
        ...
        assert!(
            !primary.contains("{--"),
            "Primary content should not contain raw CriticMarkup: {}",
            primary
        );
    ...
    async fn read_plain_doc_no_footer() {
        let server =
            build_test_server(&[("/Doc.md", "uuid-doc", "Just plain text, no markup.")]).await;
        let sid = setup_session_with_read(&server, &format!("{}-uuid-doc", RELAY_ID));
        let result = execute(
            &server,
            &sid,
...
mod tests {
    ...
    async fn read_reloads_doc_from_store_after_gc_eviction() {
        ...
        let sid = server
            .mcp_sessions
            .create_session("2025-03-26".into(), None, y_sweet_core::share_token::McpAccess { writable: true, folder_uuid: None, folder_name: None });

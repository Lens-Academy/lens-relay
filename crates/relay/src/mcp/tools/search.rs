...
mod tests {
    ...
    async fn search_no_index_returns_error() {
        ...
        let result = execute(&server, &json!({"query": "photosynthesis"})).await;
    ...
    async fn search_empty_query_returns_no_results() {
        ...
        let result = execute(&server, &json!({"query": "  "})).await;
    ...
    async fn search_with_index() {
        ...
        let search_index =
            Arc::new(y_sweet_core::search_index::SearchIndex::new(tmp.path()).unwrap());
        ...
        let result = execute(&server, &json!({"query": "photosynthesis"}))
            .await
            .unwrap();

        assert!(
            result.contains("1 result"),
            "Expected 1 result, got: {}",
            result
        );
        assert!(
            result.contains("Photosynthesis"),
            "Expected title in output, got: {}",
            result
        );
        assert!(
            result.contains("Lens"),
            "Expected folder in output, got: {}",
            result
        );
    }
    ...
    async fn search_respects_limit() {
        ...
        let result = execute(&server, &json!({"query": "biology", "limit": 2}))
            .await
            .unwrap();

        assert!(
            result.starts_with("2 result"),

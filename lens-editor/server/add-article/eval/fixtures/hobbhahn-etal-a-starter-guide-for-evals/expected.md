...
If a malicious actor wanted to get a model to act in specific (bad) ways, they would likely fine-tune the model rather than just prompting it. The same is true for a misaligned model that wanted to self-improve for nefarious purposes. Therefore, model evaluators should conduct gain-of-function research in [controlled and safe environments](https://www.apolloresearch.ai/blog/security) to elicit these behaviors. Fine-tuning can also be helpful in other ways, e.g. to test how easy it is to undo the guardrails of the model or fine-tune a specialized helper model for a more complex task.
...
To get started, you can check out guides by [Lakera](https://www.lakera.ai/blog/llm-fine-tuning-guide), [Maya Akim](https://medium.com/@mayaakim/complete-guide-to-llm-fine-tuning-for-beginners-c2c38a3252be), [Hugging Face](https://huggingface.co/docs/transformers/training), or the [OpenAI finetuning API](https://platform.openai.com/docs/guides/fine-tuning).
...
To get started, you can check out tutorials from [Hugging Face](https://huggingface.co/blog/rlhf), [Weights and Biases](https://wandb.ai/ayush-thakur/RLHF/reports/Understanding-Reinforcement-Learning-from-Human-Feedback-RLHF-Part-1--VmlldzoyODk5MTIx), and [Labellerr](https://www.labellerr.com/blog/reinforcement-learning-from-human-feedback/).
...
To get started, you can check out a [blog post by Lilian Weng](https://lilianweng.github.io/posts/2023-06-23-agent/) and [METR’s paper](https://arxiv.org/abs/2312.11671). 

### Tool use

There are a couple of tools that might be helpful to know for evals, e.g. [Langchain](https://www.langchain.com/) or the [OpenAI evals framework](https://github.com/openai/evals). Both of these are early-stage packages and come with their own flaws but they might still be helpful in your workflow or inspire you to write better ones.
...
- You can do programs like [MATS](https://www.matsprogram.org/) outside of university. Often there are great benefits from being in an environment where many people are working on related projects and can discuss their experiences and findings with each other.
...
While there is some helpful theoretical knowledge to be learned about software engineering (see e.g. [A Philosophy of Software Design](https://www.amazon.co.uk/Philosophy-Software-Design-John-Ousterhout/dp/1732102201) or [Clean Code](https://www.amazon.com/Clean-Code-Handbook-Software-Craftsmanship/dp/0132350882)), we expect most benefits to come from practical experience. Fortunately, with modern LLMs, learning software engineering has become easier than ever, both because you can iterate faster and because you can use LLMs to provide feedback on your code and suggest general improvements.
...
We shared this post with participants of the [Apart Research Evals Hackathon](https://alignmentjam.com/jam/evaluations) on 24 November 2023 and are thankful for feedback from participants.
...
export function EditorArea({ currentDocId }: { currentDocId: string }) {
  ...
  const { rightSidebarRef, rightCollapsed, setRightCollapsed, commentMarginRef, commentMarginCollapsed, setCommentMarginCollapsed, discussionRef, setDiscussionCollapsed, headerStage } = useSidebar();
    ...
    <main className="h-full flex flex-col min-h-0">
      ...
      <div ref={innerRef as RefObject<HTMLDivElement>} className="flex-1 flex min-h-0">
        <Group id="editor-area" className={`flex-1 min-h-0${isDragging ? ' panels-dragging' : ''}`}>
          ...
          <Panel id="right-sidebar" order={2} panelRef={rightSidebarRef} defaultSize="18%" minSize={`${rightMinPercent}%`} collapsible collapsedSize="0%" onResize={(size) => setRightCollapsed(size.asPercentage === 0)}>

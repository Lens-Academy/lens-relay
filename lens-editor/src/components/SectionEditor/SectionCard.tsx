...
const SECTION_COLORS: Record<string, string> = {
  ...
  callout: "bg-orange-50 border-orange-200",
  "end-callout": "bg-orange-50 border-orange-200",
...
export function SectionCard({
  ...
  const body = (
    section.type === "frontmatter" ? lines.slice(1, -2) : lines.slice(1)
  )
    .join("\n")
    .trim();
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-4 py-2 border-b border-inherit">
        ...
        {docLabel && docColor && (
          <span
            className={`text-xs px-1.5 py-0.5 rounded ${docColor.bg} ${docColor.text}`}
          >
            {docLabel}
          </span>
        )}

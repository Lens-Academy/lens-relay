...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Create tool modules**
    ...
    For forward links:
    - Get the content doc via `server.docs().get(&doc_info.doc_id)`
    - Read Y.Text("contents"), get_string into owned String, drop guards
    - Call `link_parser::extract_wikilinks(&content)` to get link names
    - For each link name, resolve to UUID: iterate all folder docs' filemeta_v0 using the same resolve_links_to_uuids pattern from link_indexer. Since resolve_links_to_uuids is private, reimplement the resolution inline or create a helper: for each link name, use doc_resolver's path_to_doc map to find matching paths (case-insensitive basename match of link name against path basenames). This is simpler than the filemeta approach since we already have all paths cached.
    - Actually, better approach: extract wikilinks, then for each link name check doc_resolver paths. A wikilink like `[[Photosynthesis]]` should match any path ending in `/Photosynthesis.md` or equal to `{Folder}/Photosynthesis.md`. Iterate all_paths(), find matches by basename (strip folder prefix and .md suffix, case-insensitive compare). First match wins per link.
    - Resolve matched paths to their full path string
    - Deduplicate forward links
...
<task type="auto">
  ...
  <action>
    ...
    **Test sequence (curl):**
    ...
    ```
    Expect: result.tools array with 3 entries (read, glob, get_links).
    ...
    ```
    Expect: result with content[0].text containing document paths, isError: false.
    ...
    ```
    Expect: result with content[0].text in cat -n format (6-char right-aligned line numbers, tab, content), isError: false.
    ...
    ```
    Expect: result with isError: true, content[0].text contains "not found".
    ...
    ```
    Expect: result with content[0].text containing "Backlinks" and "Forward links" sections, isError: false.
    ...
    ```
    Expect: result with isError: true (tool-level error, NOT JSON-RPC error).
  ...
  <verify>
    All 8 curl tests produce expected responses. Specifically:
    - tools/list returns exactly 3 tools with correct names
    - glob returns document paths (at least 1 document)
    - read returns cat -n formatted content
    - read with bad path returns isError: true
    - get_links returns backlinks/forward links sections
    - Unknown tool returns isError: true (not JSON-RPC error)
  </verify>
  <done>

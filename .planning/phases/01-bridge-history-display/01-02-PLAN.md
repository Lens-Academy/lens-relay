...
must_haves:
  ...
  artifacts:
    ...
    - path: "lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json"
      provides: "Real Discord API message response for test fixtures"
    - path: "lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json"
      provides: "Real Discord API channel info response for test fixtures"
  key_links:
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <resume-signal>Type "approved" if the bridge returns real Discord messages, or describe any issues. Also provide the channel ID you tested with -- it will be used to capture fixture data in the next task.</resume-signal>
</task>

<task type="auto">
  <name>Task 4: Capture real Discord API responses as test fixtures</name>
  <files>
    lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json
    lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json
  </files>
  <action>
    After the human checkpoint confirms the sidecar works with real Discord, capture the REAL API responses as fixture files for Plan 01-03's tests. This ensures mock data in tests is a genuine API response shape, not guesswork.

    The sidecar must still be running from the checkpoint step. Use the channel ID the user verified with.

    1. Create the `__fixtures__/` directory:
       ```bash
       mkdir -p lens-editor/src/components/DiscussionPanel/__fixtures__
       ```

    2. Capture messages (use the channel ID from the checkpoint -- it will be in the resume-signal response or conversation context):
       ```bash
       curl -s http://localhost:8091/api/channels/CHANNEL_ID/messages?limit=50 | jq '.' > lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json
       ```

    3. Capture channel info:
       ```bash
       curl -s http://localhost:8091/api/channels/CHANNEL_ID | jq '.' > lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json
       ```

    4. Validate the captured data is non-empty and has the expected shape:
       ```bash
       # Messages should be a non-empty JSON array with id, content, author, timestamp
       jq '.[0] | keys' lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json
       # Channel should have id, name, type
       jq 'keys' lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json
       ```

    5. If `jq` is not installed, use `python3 -m json.tool` instead for formatting, or save raw and manually verify the JSON structure.

    **IMPORTANT:** These fixture files contain REAL Discord data (usernames, message content). This is fine for test fixtures in a private repo. The data should have at least 5-10 messages for meaningful test coverage of grouping behavior and various message types.
  </action>
  <verify>
    ```bash
    # Fixture files exist and are valid JSON
    jq empty lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json
    jq empty lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json

    # Messages fixture is a non-empty array
    jq 'length > 0' lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json

    # Messages have expected fields
    jq '.[0] | has("id", "content", "author", "timestamp")' lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json

    # Channel has expected fields
    jq 'has("id", "name")' lens-editor/src/components/DiscussionPanel/__fixtures__/discord-channel.json
    ```
  </verify>
  <done>Two fixture JSON files exist containing real Discord API responses. Messages fixture is a non-empty array where each item has id, content, author (with id, username, avatar), and timestamp. Channel fixture has id and name. These fixtures will be used by Plan 01-03's component tests.</done>
</task>
...
- Fixture files captured at `lens-editor/src/components/DiscussionPanel/__fixtures__/discord-messages.json` and `discord-channel.json` containing real Discord API responses
</success_criteria>
